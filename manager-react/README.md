...
> **대표(owner) 전용 화면**: Home·팀원 현황은 화면 성격이 완전히 달라 owner일 때
> 별도 컴포넌트(`CeoDashboard`/`CeoCompanyStatus`)로 교체되고, Report Center는
> owner에게만 노출됩니다. (프로세스는 대표도 팀장과 동일한 `ProcessPage`를 사용 — 대표 전용 컴포넌트 폐기) 데이터는 한 벌(`state.js`)을 공유하며, 카드/섹션 UI는 `CeoUI.jsx`에
