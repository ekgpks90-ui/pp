...
function todayDayIndex() {
  return (new Date(TODAY_ISO).getDay() + 6) % 7
}
...
export default function WeeklyTasks({ weekOffset, searchQuery, workItems, sessions = [], onDateClick, onAddSession, onDeleteWorkItem, onOpenDetail, onOpenTaskDrawer }) {
  ...
  const todayIdx = weekOffset === 0 ? todayDayIndex() : -1
  const [openSet, setOpenSet] = useState(() => new Set([todayIdx]))
    ...
    <div className="bg-surface border border-line rounded-[14px] flex flex-col overflow-hidden shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-2 p-[10px_14px_14px]">
        {dayData.map((day, i) => {
          ...
          const lastFixedIdx = day.items.reduce((acc, item, j) => item.type === '고정' ? j : acc, -1)
          ...
          return (
            <div key={day.date} className={`rounded-[10px] shrink-0 ${isToday ? 'bg-blue-soft border border-blue-mid' : 'bg-surface-muted border border-line-soft'}`}>
              ...
              <button
                type="button"
                onClick={() => toggle(i)}
                className="w-full flex items-center gap-[7px] px-[14px] pt-3 pb-2 cursor-pointer"
              >
                <span className={`text-[13.5px] font-bold tracking-[-0.02em] ${isToday ? 'text-blue' : 'text-text-primary'}`}>{day.label}</span>
                <span
                  className={`text-[12px] ${isToday ? 'text-blue' : 'text-soft'}`}
                ...
                {day.items.length > 0 && (
                  <span className={`text-[10px] font-semibold ml-0.5 ${isToday ? 'text-blue/60' : 'text-soft'}`}>{day.items.length}</span>
                )}
                <svg
                  width="12" height="12" viewBox="0 0 12 12" fill="none"
                  className={`ml-auto shrink-0 transition-transform duration-200 ${isToday ? 'text-blue/60' : 'text-muted'} ${isOpen ? 'rotate-180' : ''}`}
                >
