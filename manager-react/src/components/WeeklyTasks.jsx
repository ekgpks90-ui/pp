...
export default function WeeklyTasks({ weekOffset, searchQuery, workItems, sessions = [], onDateClick, onAddSession, onDeleteWorkItem, onOpenDetail, onOpenTaskDrawer }) {
  ...
  const [selectedIdx, setSelectedIdx] = useState(() => weekOffset === 0 ? todayDayIndex() : 0)
  ...
  return (
    <div className="bg-surface border border-line rounded-[14px] flex flex-col overflow-hidden shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none h-7 pl-3 pr-7 text-[12px] font-semibold border border-line rounded-lg bg-white text-text-primary outline-none focus:border-blue cursor-pointer"
            >
              {dayData.map((d, i) => (
            ...
            <svg className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-muted" width="10" height="10" viewBox="0 0 10 10" fill="none">
      ...
      {/* Selected day */}
      <div className="flex-1 overflow-y-auto p-[10px_14px_14px]">
        <div className={`rounded-[10px] ${isToday ? 'bg-blue-soft border border-blue-mid' : 'bg-surface-muted border border-line-soft'}`}>
