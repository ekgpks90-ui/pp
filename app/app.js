...
function _mpAI() {
  ...
  const allInsights=[
    ...
    {icon:'⏱️',label:'가장 많은 시간이 투입된 업무',value:(topWi&&topWi[1]>0)?`${topWi[0]} (${fmtH(topWi[1])})`:null},
    {icon:'🔁',label:'반복 업무 패턴',            value:repeated.length?repeated.slice(0,2).join(', '):null},
    {icon:'🌟',label:'생산성이 높은 요일',         value:(bestDay&&bestDay[1]>0)?`${fmtDate(bestDay[0])} (${fmtH(bestDay[1])})`:null},
    {icon:'📉',label:'생산성이 낮은 요일',         value:(worstDay&&worstDay[1]>0&&worstDay!==bestDay)?`${fmtDate(worstDay[0])} (${fmtH(worstDay[1])})`:null},
...
function submitScheduleMeeting(e) {
  ...
  // 등록 완료 알림
  const otherAttendees = attendees.filter(name => name !== state.currentUser.name);
  const notifyMsg = otherAttendees.length > 0
    ? `[${room}] ${title} 등록 완료. ${otherAttendees.length}명에게 알림 발송.`
    : `[${room}] ${title} 등록되었습니다.`;
  addNotification('일반', '회의 등록 완료', notifyMsg, title);
